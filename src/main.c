...
main (int argc, char *argv[])
{
	...
	msgid_7311 = msgget ((key_t) 1238, 0666 | IPC_CREAT);
	if (msgid_7311 == -1)
	{
	  ...
	  remotetimer_7311 = g_timeout_add (500, (GSourceFunc) remote_entry_7311, NULL);

#endif

	/* Socket Server AF_INET Setup 7311 */
#ifndef G_OS_WIN32
	result = remote_socket_setup_7311();
	if (result == -1)
	{
		temp = g_strdup_printf (_("Socket setup UDP Port 7311 failed: %s"), g_strerror (errno));
		update_statusbar (temp);
		close (server_sockfd_7311);
	}
	else /* check for a message on the socket */
	{
		channel_7311 = g_io_channel_unix_new (server_sockfd_7311);
		sockettimer_7311 = g_io_add_watch (channel_7311, G_IO_IN, socket_entry_7311, NULL);
	}
#endif
	/* Socket Server 2333 */
#ifndef G_OS_WIN32
	result = remote_socket_setup_2333();
	if (result == -1)
	{
		temp = g_strdup_printf (_("Socket setup UDP Port 2333 failed: %s"), g_strerror (errno));
		update_statusbar (temp);
		close (server_sockfd_2333);
	}
	else /* check for a message on the socket */
	{
		channel_2333 = g_io_channel_unix_new (server_sockfd_2333);
		flags_2333 = g_io_channel_get_flags(channel_2333);
		g_io_channel_set_flags(channel_2333, flags_2333 | G_IO_FLAG_NONBLOCK, NULL);
		g_io_channel_set_line_term(channel_2333, ">", -1);
		sockettimer_2333 = g_io_add_watch (channel_2333, G_IO_IN, socket_entry_2333, NULL);
	}
#endif
	/* hamlib stuff */
