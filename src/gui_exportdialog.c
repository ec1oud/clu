/*
   ...
   Copyright (C) 2012 - 2017 Andy Stewart <kb1oiq@arrl.net>
 ...
 * adif v3, cabrillo v3, EDI, trlog (tlf) and tab separated value (TSV) for
...
on_menu_export_activate (GtkMenuItem * menuitem, gpointer user_data)
{
	...
	if (page >= 0)
	{
		...
		gtk_combo_box_append_text (GTK_COMBO_BOX (filemenu), "adif v3");
		...
		if (export_response == GTK_RESPONSE_OK)
		{
			...
			{
			    exportwarningdialog = gtk_dialog_new_with_buttons (_("xlog - exporting"),
					GTK_WINDOW(mainwindow), GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT,
					GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL,
					GTK_STOCK_OK, GTK_RESPONSE_OK, NULL);
				temp = g_strdup_printf (_("%s already exists, do you want to overwrite it?"), sellog);
				exportwarninglabel = gtk_label_new (temp);
				gtk_container_add (GTK_CONTAINER (GTK_DIALOG (exportwarningdialog)->vbox), exportwarninglabel);
				...
				exportwarning_response = gtk_dialog_run (GTK_DIALOG(exportwarningdialog));
