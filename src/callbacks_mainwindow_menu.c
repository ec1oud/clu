...
on_sort_log_callsign_ascending_activate (GtkMenuItem *menuitem, gpointer user_data)
{
  ...
  if (sorted_by_call == FALSE) {
	...
	column = gtk_tree_view_get_column (GTK_TREE_VIEW(logw->treeview), CALL);
	gtk_tree_view_column_set_clickable (column, TRUE);

	gtk_tree_view_column_set_sort_column_id (column, CALL);
	model = gtk_tree_view_get_model (GTK_TREE_VIEW(logw->treeview));
	gtk_tree_sortable_set_sort_func (GTK_TREE_SORTABLE (model), CALL, sort_by_callsign_ascending, NULL, NULL);
