...
{
	GtkWidget *logeditor,
	  *endhbox, *awardshbox, *hrsthbox, *mrsthbox, *powerhbox, *qslhbox, *namehbox, *qthhbox,
		...
		*lehbox7, *lelabel7_1, *lecombo7, *lee7,
		*lehbox8, *lelabel8_1, *lecombo8, *lee8,
	...
	if (page >= 0)
	{
		...
		/* AndyS 26-dec-2020 */
		/* Make the TX(RST) and RX(RST) selectable:  Show/Hide */
		/* These aren't used during VHF contests, for example. */
		...
		gtk_box_pack_start (GTK_BOX (vboxleft), lehbox7, FALSE, FALSE, 0);
		gtk_widget_set_size_request (lehbox7, -1, 30);
		lelabel7_1 = gtk_label_new (_("TX(RST)"));
		gtk_box_pack_start (GTK_BOX (lehbox7), lelabel7_1, FALSE, FALSE, 0);
		lecombo7 = gtk_combo_box_new_text ();
		gtk_box_pack_start (GTK_BOX (lehbox7), lecombo7, FALSE, FALSE, 0);
		gtk_combo_box_append_text (GTK_COMBO_BOX (lecombo7), _("Show"));
		gtk_combo_box_append_text (GTK_COMBO_BOX (lecombo7), _("Hide"));
		lee7 = gtk_entry_new ();
		gtk_widget_set_size_request (lee7, 50, -1);
		gtk_box_pack_start (GTK_BOX (lehbox7), lee7, FALSE, FALSE, 0);
		temp = g_strdup_printf ("%d", preferences.logcwidths2[RST]);
		...
		gtk_box_pack_start (GTK_BOX (vboxleft), lehbox8, FALSE, FALSE, 0);
		gtk_widget_set_size_request (lehbox8, -1, 30);
		lelabel8_1 = gtk_label_new (_("RX(RST)"));
		gtk_box_pack_start (GTK_BOX (lehbox8), lelabel8_1, FALSE, FALSE, 0);
		lecombo8 = gtk_combo_box_new_text ();
		gtk_box_pack_start (GTK_BOX (lehbox8), lecombo8, FALSE, FALSE, 0);
		gtk_combo_box_append_text (GTK_COMBO_BOX (lecombo8), _("Show"));
		gtk_combo_box_append_text (GTK_COMBO_BOX (lecombo8), _("Hide"));
		lee8 = gtk_entry_new ();
		gtk_widget_set_size_request (lee8, 50, -1);
		gtk_box_pack_start (GTK_BOX (lehbox8), lee8, FALSE, FALSE, 0);
		temp = g_strdup_printf ("%d", preferences.logcwidths2[MYRST]);
		...
		mrsthbox = lookup_widget (mainwindow, "mrsthbox");
		if (gtk_widget_get_visible (mrsthbox))
			gtk_combo_box_set_active (GTK_COMBO_BOX (lecombo7), 0);
		else
			gtk_combo_box_set_active (GTK_COMBO_BOX (lecombo7), 1);

		hrsthbox = lookup_widget (mainwindow, "hrsthbox");
		if (gtk_widget_get_visible (hrsthbox))
			gtk_combo_box_set_active (GTK_COMBO_BOX (lecombo8), 0);
		else
			gtk_combo_box_set_active (GTK_COMBO_BOX (lecombo8), 1);
		...
		if (response == GTK_RESPONSE_OK)
		{
			...
			index = gtk_combo_box_get_active (GTK_COMBO_BOX (lecombo7));
			column = gtk_tree_view_get_column (GTK_TREE_VIEW (logw->treeview), RST);
			if (index == 1)
				gtk_tree_view_column_set_visible (GTK_TREE_VIEW_COLUMN(column), FALSE);
			else
				gtk_tree_view_column_set_visible (GTK_TREE_VIEW_COLUMN(column), TRUE);
			update_logfields (RST, fields, index);

			index = gtk_combo_box_get_active (GTK_COMBO_BOX (lecombo8));
			column = gtk_tree_view_get_column (GTK_TREE_VIEW (logw->treeview), MYRST);
			if (index == 1)
				gtk_tree_view_column_set_visible (GTK_TREE_VIEW_COLUMN(column), FALSE);
			else
				gtk_tree_view_column_set_visible (GTK_TREE_VIEW_COLUMN(column), TRUE);
			update_logfields (MYRST, fields, index);
