...
on_menu_preferences_activate (GtkMenuItem * menuitem, gpointer user_data)
{
	...
	gtk_entry_set_max_length (GTK_ENTRY (modesentry), 1024);
	...
	remotedatalabel = gtk_label_new
		(_("When receiving data from another application (gmfsk, ktrack, fldigi, wsjt-x):"));
	gtk_box_pack_start
...
(_("Initial exchange to use for \"~\" (see the manual for details)"));
	...
	gtk_combo_box_append_text (GTK_COMBO_BOX (devicecombo), "/dev/ttyUSB0");
	gtk_combo_box_append_text (GTK_COMBO_BOX (devicecombo), "/dev/ttyUSB1");
...
#endif
	gtk_entry_set_max_length (GTK_ENTRY (GTK_BIN(devicecombo)->child), 125);
