/*
   ...
   Copyright (C) 2016 - 2021 Andy Stewart <kb1oiq@arrl.net>
...
start_hamlib (gint rigid, gchar *device, gint debugmode, gint timervalue)
{
	...
	{
		temp = g_strdup_printf
			(_("An error occurred while opening port %s: %s"),
...
get_powerlevel (void)
{
	...
	if (gtk_widget_get_visible (powerhbox))
	{
		...
		// If hamlib cannot return a power level, don't cause an error.
		// The "Power" field in the New QSO pane will simply display UNKNOWN.
...
get_smeter (void)
{
  ...
  // If getting the smeter reading fails, the code will simply
  // ignore that error.
  if (retcode != RIG_OK)
	strength.i = 0;  // pretend S9
...
GString *convert_frequency (void)
{
	...
	if (preferences.round == 0)
		g_string_printf (digits, "%lld", programstate.rigfrequency);
	else
		g_string_printf (digits, "%lld", (long long) rint (programstate.rigfrequency /
