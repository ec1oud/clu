 ...
 * Specification:  https://wwrof.org/cabrillo/
 */
...
cabrillo3_open (LOGDB * handle)
{
    ...
    // printf("DEBUG: cabrillo3.c:  cabrillo3_open (): got here #1\n");
...
cabrillo3_create (LOGDB * handle)
{
    ...
    // printf("DEBUG: cabrillo3.c:  cabrillo3_create (): got here #1\n");
...
cabrillo3_close (LOGDB * handle)
{
    ...
    // printf("DEBUG: cabrillo3.c:  cabrillo3_close (): got here #1\n");
...
cabrillo3_qso_append (LOGDB * handle, const qso_t * q)
{
    ...
    /* If the QSO frame has TX/RX RST hidden, just put in 59(9) */

    if (q[RST] == NULL) {
	  strncpy(rst, "599", rst_len);
	  rst[rst_len] = '\0';
	}  else {
    ...
    if (q[MYRST] == NULL) {
	  strncpy(my_rst, "599", rst_len);
	  my_rst[rst_len] = '\0';
	}  else {
	  strncpy (my_rst, q[RST], rst_len);
	  my_rst[rst_len] = '\0';
	...
	/* translate mode, valid: PH, RY, FM, CW, DG */
	if (!strcmp (q_mode, "USB") ||
	    !strcmp (q_mode, "LSB") ||
		!strcmp (q_mode, "SSB") ||
		!strcmp (q_mode, "AM"))
		{
	...
	// AndyS: Should all digital modes be represented as DG?
    // How does that dovetail with this use of unsupported modes used
    // in Europe?

    // Leave these 3 modes alone....all others are DG.
	...
	gint (*fn) (LOGDB *, qso_t *, gpointer arg), gpointer arg)
{
    ...
    // printf("DEBUG: cabrillo3.c:  cabrillo3_qso_foreach (): got here #1\n");
