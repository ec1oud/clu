...
bool remove_prefixes (gchar * key, gpointer value, gpointer user_data);
void save_windowsize_and_cleanup (void);
gchar *xloggetdate (void);
gchar *xloggettime (void);
gchar *lookup_mode (int index);
gchar *lookup_band (int index);
...
gchar *valid_awards_entry (gchar *a, uint *st, uint *zone, uint *cont, uint *iota);
bool qslreceived(gchar *in);

/* used only for EDI export (so far) */
int get_num_qsos_to_export(void);
void save_num_qsos_to_export(int num);
